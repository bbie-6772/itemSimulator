...
const signVaild = async function (req, res, next) {
    try {
        ...
        if (!/^[a-z0-9]*$/.test(id))
            throw new Error('아이디는 소문자와 숫자로만 입력해주세요', {
                cause: 400,
            });

        // 비밀번호가 6글자 이상인지 확인
        if (!/\b.{6,}/.test(password))
            throw new Error('비밀번호는 6글자 이상으로 작성해주세요', {
                cause: 400,
            });
        ...
        if (err.cause) next(err);
            ...
            console.error(err);
            next(
                new Error('<middleware> : signVaild 잘못된 접근입니다.', {
                    cause: 400,
                })
            );
        }
...
const charVaild = async function (req, res, next) {
    try {
        ...
        if (!charId || !Number.isInteger(+charId))
            throw new Error(
                '선택할 <character_id>를 URL에 숫자로 입력해주세요.',
                { cause: 400 }
            );
        ...
        if (!character)
            throw new Error(
                `<character_id> ${charId} 에 해당하는 캐릭터가 존재하지 않습니다.`,
                { cause: 404 }
            );
        ...
        if (user) {
            if (character.accountId !== user.accountId)
                throw new Error('본 계정이 소유한 캐릭터가 아닙니다.', {
                    cause: 401,
                });
        }
        ...
        if (err.cause) next(err);
            ...
            console.error(err);
            next(
                new Error('<middleware> : charVaild 잘못된 접근입니다.', {
                    cause: 400,
                })
            );
        }
    ...
    try {
        ...
        if (Array.isArray(body)) {
            for (const itemInfo of body) {
                ...
                if (
                    !itemInfo.item_code ||
                    !Number.isInteger(+itemInfo.item_code)
                )
                    throw new Error(
                        '선택할 <item_code> 를 숫자로 입력해주세요.',
                        { cause: 400 }
                    );

                // 아이템 유효성 평가
                const item = await prisma.itemTable.findFirst({
                    where: { itemCode: +itemInfo.item_code },
                });
                if (!item)
                    throw new Error(
                        `<item_code> ${itemInfo.item_code}번의 아이템이 존재하지 않습니다`,
                        { cause: 404 }
                    );
            }
            next();
