...
router.post('/sign-up', signVaild, async (req, res, next) => {
    try {
        ...
        if (isExitUser)
            throw new Error('이미 존재하는 아이디입니다', { cause: 409 });

        // 비밀번호 확인이 없을 시
        if (!passwordCheck)
            throw new Error('비밀번호 확인용 <passwordCheck>를 입력해주세요', {
                cause: 400,
            });

        //비밀번호 확인과 일치하는지
        if (!(password === passwordCheck))
            throw new Error('비밀번호가 일치하지 않습니다', { cause: 401 });
...
router.post('/sign-in', signVaild, async (req, res, next) => {
    try {
        ...
        if (!account)
            throw new Error('존재하지 않는 아이디입니다.', { cause: 404 });

        // 비밀번호 검증
        if (!(await bycrpt.compare(password, account.password)))
            return res
                .status(401)
