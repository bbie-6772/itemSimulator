...
router.post('/characters', authMiddleware, async (req, res, next) => {
    try {
        ...
        // 이름 존재 유무
        if (!name)
            throw new Error('캐릭터 이름 <name>을 입력해주세요', {
                cause: 400,
            });

        // 이름 중복 여부
        const isExitChar = await prisma.characters.findFirst({
            where: { name },
        });
        if (isExitChar)
            throw new Error('이미 존재하는 이름입니다', { cause: 409 });

        //트랜잭션 사용으로 캐릭터/인벤토리/장비 동시 생성
        const [character, inventory, equipment] = await prisma.$transaction(
            async (tx) => {
                //캐릭터 생성
...
router.delete(
    ...
    async (req, res, next) => {
        ...
        return res
            .status(200)
            .json({
                message: `캐릭터 ${character.name}(을)를 삭제하였습니다.`,
            });
    }
);
